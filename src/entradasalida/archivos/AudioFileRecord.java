package entradasalida.archivos;

import java.io.*;

import edlineal.DatosLista;
import edlineal.DatosListaEstaticaN;
import librerias.*;

import javax.management.ObjectName;

public class AudioFileRecord {
    private long numFrames;  //numero de tramas, número de muestras totales del archivo por canal
    private long sampleRate; //numero de muestras por segundo a la que se discretiza la señal
    private int numChannels; //número de canales
    private double[] buffer; //audio original
    private double[] buffer2; //audio modificado
    private String archivo;   //nombre de archivo dado por el usuario
    private WavFile wavFileR; //apuntador de archivo leido
    private WavFile wavFileW;  //apuntador de archivo a escribir
    private String nomArchivo; //nombre archivo (uno.wav)
    private String nomRuta;    //ruta donde esta guardado el archivo (/home)
    private int validBits;     //bits usados para la discretización
    private DatosListaEstaticaN listaN; //lista para manipular los datos

    public AudioFileRecord(String archivo) {
        this.archivo = archivo;
        // Abre el archivo wav y asigna parámetros a las variables
        try {
            File file = new File(archivo);
            wavFileR = WavFile.openWavFile(file);
            nomArchivo = file.getName();
            nomRuta = file.getParent();
        } catch (Exception e) {

        }
        numChannels = wavFileR.getNumChannels();
        numFrames = wavFileR.getNumFrames();
        sampleRate = wavFileR.getSampleRate();
        validBits=wavFileR.getValidBits();
    }


    public DatosListaEstaticaN bufferAListaNum(){
        listaN = new DatosListaEstaticaN(buffer2.length);
        for(int posicion = 0; posicion < buffer2.length; posicion++){
            listaN.agregar(buffer2[posicion]); //agregar los valores al arreglo
        }
        return listaN; //regresar el arreglo
    }

    public boolean numAListaBuffer(){
        buffer2 = new double[listaN.cantidad()];
        for(int posicion = 0; posicion < buffer2.length; posicion++){ //reccorremos la lista
            buffer2[posicion] = (double)listaN.obtener(posicion); //poner los valores de listaN en buffer2
        }
        return true; //regresar true
    }

    public void subirVolumen(int intensidad){
        //validar que la listaN no esté vacia
        if(listaN.vacia() == true){
            return;
        }
        //validar que la intensidad sea positica
        if(intensidad <= 0){
            return;
        }
        //llamar de listaN el metodo porEscalar para multiplicar por la intensidad
        double escalar = 1.0 + (intensidad/10.0); //cambiar la intensidad poe un valor double positivo
        listaN.porEscalar(escalar);
    }

    public void bajarVolumen(int intensidad){
        if(listaN.vacia() == true){
            return;
        }
        if(intensidad <= 0){
            return;
        }
        double escalar = 1.0 / (1.0 + (intensidad / 10.0)); //cambiar la intensidad poe un valor double negativo
        listaN.porEscalar(escalar);
    }

    public void acelerarAudio(int incremento){
        //validar lista no vacia
        if(listaN.vacia() == true){
            return;
        }

        if(incremento <= 1){
            return;
        }

        int cantidad = listaN.cantidad();
        DatosListaEstaticaN listaAcelerada = new DatosListaEstaticaN(cantidad);
        for(int posicion = 0; posicion < cantidad; posicion += incremento){
            listaAcelerada.agregar(listaN.obtener(posicion));
        }
        listaN = listaAcelerada;
    }

    public void retrasarAudio(int decremento){
        if(listaN.vacia() == true){
            return;
        }
        if(decremento <= 1){
            return;
        }
        int cantOriginal = listaN.cantidad();
        int nuevaCantidad = cantOriginal * decremento;

        DatosListaEstaticaN listaRetrasada = new DatosListaEstaticaN(nuevaCantidad);

        for(int posicion = 0; posicion < cantOriginal; posicion++){
            Object valor = listaN.obtener(posicion);
            for(int vecesDec = 0; vecesDec < decremento; vecesDec++){
                listaRetrasada.agregar(valor);
            }
        }
        listaN = listaRetrasada;
    }

public void eliminarSilencio(){
        if(listaN.vacia() == true){
            return;
        }
        //rango de silencio en el audio
        double silencioMin = -0.02;
        double silencioMax = 0.02;

        for(int posicion = listaN.cantidad() - 1; posicion >= 0; posicion--){
            double valor = Double.parseDouble(listaN.obtener(posicion).toString());

            if(valor >= silencioMin && valor <= silencioMax){
                listaN.eliminar(posicion);
            }
        }
}

    public void invertirAudio(){
            if(listaN.vacia() == true){
                return;
            }
            int cantidad = listaN.cantidad();
            DatosListaEstaticaN listaInvertida = new DatosListaEstaticaN(cantidad);
            //recorrer desde el final hasta el principio
            for(int posicion = cantidad -1; posicion >= 0; posicion--){
                Object valor = listaN.obtener(posicion);
                listaInvertida.agregar(valor);
            }
            listaN = listaInvertida;
    }

    public void invertirIntensidad(){
        if(listaN.vacia() == true){
            return;
        }
        listaN.porEscalar(-1.0);
    }






    public void leerAudio() {
        try {

            // Muestra datos del archivo
            wavFileR.display();

            // Crea buffer de origen y de temporal
            buffer = new double[(int) numFrames * numChannels];
            buffer2 = new double[buffer.length];

            //tramas leidas
            int framesRead;

            // Lee tramas totales
            framesRead = wavFileR.readFrames(buffer, (int) numFrames);

            // Recorrer todas las tramas del archivo y guardarlas en el arreglo.
            for (int s = 0; s < framesRead * numChannels; s++) {
                buffer2[s] = buffer[s];
            }

            // Cierra archivo
            wavFileR.close();
        } catch (Exception e) {
            System.err.println(e);
        }
    }

    public void EscribirAudio() {
        try {

            //Crear el apuntador al archivo para guardar datos del temporal
            File file = new File(nomRuta + "/2_" + nomArchivo);

            // Creamos un nuevo archivo de audio con los mismos datos que el original
            wavFileW = WavFile.newWavFile(file, numChannels, numFrames, validBits, sampleRate);

            // Escribimos los frames o muestras totales del temporal
            wavFileW.writeFrames(buffer2, (int) numFrames);

            // Cerramos el archivo
            wavFileW.close();
        } catch (Exception e) {
            System.err.println(e);
        }
    }
}
